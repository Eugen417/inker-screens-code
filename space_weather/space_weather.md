...
### 📌 Виджет 5: Радар Луны в окнах

Специальный виджет кругового радара, показывающий азимут луны, фазу (в виде серпа), ее высоту над/под горизонтом и положение относительно окон.
...
try {
  ...
  let az = parseFloat(attr.moon_azimuth || 0);
  let alt = parseFloat(attr.moon_altitude || 0);
  let angle = parseFloat(attr.moon_parallactic || 0);

  let cx = 160, cy = 210, R = 95;

  let getXY = (angle, radius) => {
  ...
  let moonDist;
  if (alt >= 0) {
      moonDist = R * ((90 - alt) / 90);
  } else {
      let depth = Math.min(1, Math.abs(alt) / 20); 
      moonDist = R + 26 + depth * 12; 
  }
  
  let moonPos = getXY(az, moonDist);
  
  let winMainLine = getXY(310, R + 40);
  let winMainTxt = getXY(310, R + 60);

  let winSideLine = getXY(220, R + 40);
  let winSideTxt = getXY(220, R + 60);
  
  let altStatus = alt > 0 ? `Над горизонтом` : `За горизонтом`;

  let rMoon = 11, rx = rMoon * 0.45; 
  let crescentPath = `M ${moonPos.x},${moonPos.y-rMoon} A ${rMoon},${rMoon} 0 0,1 ${moonPos.x},${moonPos.y+rMoon} A ${rx},${rMoon} 0 0,0 ${moonPos.x},${moonPos.y-rMoon} Z`;

  let moonGraphic = `
  <g transform="rotate(${angle}, ${moonPos.x}, ${moonPos.y})">
      <path d="${crescentPath}" fill="#fff" stroke="#000" stroke-width="2" stroke-linejoin="round"/>
  </g>`;

  let profX = 355, trackTop = 220, horizY = 300, trackBottom = 380;       
  let altClamped = Math.max(-90, Math.min(90, alt));
  let moonProfY;
  if (altClamped >= 0) moonProfY = horizY - (altClamped / 90) * (horizY - trackTop);
  else moonProfY = horizY + (Math.abs(altClamped) / 90) * (trackBottom - horizY);

  let calloutStart = getXY(50, R);        
  let calloutKneeX = calloutStart.x + 35; 
  let calloutKneeY = calloutStart.y - 45; 
  let calloutEndX = calloutKneeX + 80;    
  ...
  <svg xmlns="http://www.w3.org/2000/svg" width="400" height="400" viewBox="0 0 400 400">
    <style>
      .t { font-family: Arial, sans-serif; fill: #000; font-weight: bold; } 
      .s { font-family: Arial, sans-serif; fill: #000; font-weight: bold; } 
      .lbl { font-size: 12px; fill: #000; font-family: Arial, sans-serif; }
      .halo { stroke: #fff; stroke-width: 5px; paint-order: stroke; stroke-linejoin: round; }
    </style>
    
    <text x="20" y="35" font-size="24" class="t" letter-spacing="1">РАДАР: ЛУНА В ОКНАХ</text>
    <line x1="20" y1="50" x2="380" y2="50" stroke="#000" stroke-width="4" stroke-linecap="round"/>

    <circle cx="${cx}" cy="${cy}" r="${R/2}" fill="none" stroke="#000" stroke-width="1" stroke-dasharray="3,6"/>
    <circle cx="${cx}" cy="${cy}" r="${R}" fill="none" stroke="#000" stroke-width="3"/>
    
    <line x1="${cx}" y1="${cy}" x2="${winMainLine.x}" y2="${winMainLine.y}" stroke="#000" stroke-width="3" stroke-dasharray="8,5"/>
    <line x1="${cx}" y1="${cy}" x2="${winSideLine.x}" y2="${winSideLine.y}" stroke="#000" stroke-width="3" stroke-dasharray="8,5"/>

    <line x1="${cx}" y1="${cy}" x2="${moonPos.x}" y2="${moonPos.y}" stroke="#000" stroke-width="1" stroke-dasharray="4,4"/>
    
    <circle cx="${calloutStart.x}" cy="${calloutStart.y}" r="3" fill="#000"/>
    <polyline points="${calloutStart.x},${calloutStart.y} ${calloutKneeX},${calloutKneeY} ${calloutEndX},${calloutKneeY}" fill="none" stroke="#000" stroke-width="1.5"/>

    <circle cx="${cx}" cy="${cy}" r="5" fill="#000"/>

    <text x="${profX}" y="${trackTop - 20}" font-size="14" class="t" text-anchor="middle">Высота</text>
    <line x1="${profX}" y1="${trackTop}" x2="${profX}" y2="${trackBottom}" stroke="#000" stroke-width="1.5" stroke-dasharray="3,3"/>
    
    <line x1="${profX - 6}" y1="${trackTop}" x2="${profX + 6}" y2="${trackTop}" stroke="#000" stroke-width="2"/>
    <text x="${profX - 10}" y="${trackTop + 4}" font-size="11" class="s" text-anchor="end">90°</text>

    <line x1="${profX - 6}" y1="${trackBottom}" x2="${profX + 6}" y2="${trackBottom}" stroke="#000" stroke-width="2"/>
    <text x="${profX - 10}" y="${trackBottom + 4}" font-size="11" class="s" text-anchor="end">-90°</text>
    
    <line x1="${profX - 18}" y1="${horizY}" x2="${profX + 18}" y2="${horizY}" stroke="#000" stroke-width="3"/>
    <text x="${profX - 22}" y="${horizY + 4}" font-size="11" class="t" text-anchor="end">0° Горизонт</text>
    
    ${alt >= 0 ? `<circle cx="${profX}" cy="${moonProfY}" r="6" fill="#fff" stroke="#000" stroke-width="2"/>` : `<circle cx="${profX}" cy="${moonProfY}" r="6" fill="none" stroke="#000" stroke-width="2" stroke-dasharray="2,2"/>`}
    <text x="${profX + 12}" y="${moonProfY + 4}" font-size="13" class="t halo" text-anchor="start">${alt.toFixed(1)}°</text>

    <text x="${cx + 4}" y="${cy - R/2 - 4}" class="lbl halo">45°</text>
    <text x="${cx + 8}" y="${cy + 4}" class="lbl halo">Зенит</text>

    <text x="${calloutKneeX + 5}" y="${calloutKneeY - 6}" font-size="12" class="t halo">Горизонт (0°)</text>

    <text x="${cx}" y="${cy - R - 18}" class="s halo" font-size="16" text-anchor="middle">С</text>
    <text x="${cx}" y="${cy + R + 28}" class="s halo" font-size="16" text-anchor="middle">Ю</text>
    <text x="${cx - R - 25}" y="${cy + 6}" class="s halo" font-size="16" text-anchor="middle">З</text>
    <text x="${cx + R + 25}" y="${cy + 6}" class="s halo" font-size="16" text-anchor="middle">В</text>

    <text x="${winMainTxt.x}" y="${winMainTxt.y - 6}" font-size="12" class="t halo" text-anchor="middle">310°</text>
    <text x="${winMainTxt.x}" y="${winMainTxt.y + 8}" font-size="12" class="t halo" text-anchor="middle">(Осн)</text>

    <text x="${winSideTxt.x}" y="${winSideTxt.y - 6}" font-size="12" class="t halo" text-anchor="middle">220°</text>
    <text x="${winSideTxt.x}" y="${winSideTxt.y + 8}" font-size="12" class="t halo" text-anchor="middle">(Бок)</text>

    <text x="20" y="360" font-size="20" class="t">Азимут: ${az.toFixed(1)}°</text>
    <text x="20" y="390" font-size="20" class="t">Выс: ${alt.toFixed(1)}° (${altStatus})</text>
    
    ${moonGraphic}
  </svg>`;
  
  return "data:image/svg+xml;charset=utf-8," + encodeURIComponent(svg.replace(/\n\s*/g, ''));
} catch (err) { return "data:image/svg+xml;charset=utf-8," + encodeURIComponent(`<svg width="400" height="400"><text y="40" font-size="20">Ошибка: ${err.message}</text></svg>`); }
